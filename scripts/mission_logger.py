...
class MissionLogger:
    ...
    def _print_summary(self, summary: MissionSummary):
        ...
        rospy.logdebug(f"\n{'='*70}")
        rospy.logdebug(f"{'RELATÓRIO DA MISSÃO':^70}")
        rospy.logdebug(f"{'='*70}")
        rospy.logdebug(f"ID: {summary.mission_id}")
        rospy.logdebug(f"Data: {summary.date}")
        rospy.logdebug(f"")
        rospy.logdebug(f"{'--- CONFIGURAÇÃO ---':^70}")
        rospy.logdebug(f"Talhão: {summary.grid_size_x}m x {summary.grid_size_y}m")
        rospy.logdebug(f"Espaçamento: {summary.waypoint_spacing}m")
        rospy.logdebug(f"Solver: {summary.solver_name} ({summary.solver_time:.2f}s)")
        rospy.logdebug(f"")
        rospy.logdebug(f"{'--- RESULTADOS ---':^70}")
        rospy.logdebug(f"Plantas: {summary.planted_waypoints}/{summary.total_waypoints} ({summary.success_rate:.1f}%)")
        rospy.logdebug(f"Rotas: {summary.num_routes} | Recargas: {summary.recharges}")
        rospy.logdebug(f"Distância: {summary.total_distance:.1f}m")
        rospy.logdebug(f"Tempo: {mins:02d}:{secs:02d}")
        rospy.logdebug(f"")
        rospy.logdebug(f"{'--- EFICIÊNCIA ---':^70}")
        rospy.logdebug(f"Velocidade: {summary.plants_per_minute:.2f} plantas/min")
        rospy.logdebug(f"Economia: {summary.meters_per_plant:.2f} m/planta")
        rospy.logdebug(f"Bateria: {summary.total_battery_consumed:.1f}% consumido")
        rospy.logdebug(f"Consumo médio: {summary.avg_consumption_per_plant:.3f}%/planta")
        rospy.logdebug(f"Score: {summary.efficiency_score:.4f}")
        rospy.logdebug(f"{'='*70}")
        rospy.logdebug(f"Logs salvos em: {self.output_dir}")
        rospy.logdebug(f"{'='*70}\n")
